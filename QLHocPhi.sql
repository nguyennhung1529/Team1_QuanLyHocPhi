...
insert into KY_HOC_PHI (MaKyHoc, MSV, HocPhi, DaDong)
-- 
select DISTINCT HOC_PHAN.MaKyHoc, SINH_VIEN.MSV, SUM(HOC_PHAN.SoTC * TIEN_TIN.TienTC) as 'hocPhi'
from HOC_TAP
...
select HOC_PHAN.MaKyHoc, 
	...
	SUM(HOC_PHAN.SoTC * TIEN_TIN.TienTC * (1 - DOI_TUONG.MucGiam)) as 'TienCanDong'
from HOC_TAP
	join SINH_VIEN on SINH_VIEN.MSV = HOC_TAP.MSV
	join HOC_PHAN on HOC_PHAN.MaHP = HOC_TAP.MaHP
	join CT_DOI_TUONG on CT_DOI_TUONG.MSV = SINH_VIEN.MSV
	...
	join LOP on LOP.MaL = SINH_VIEN.MaL
	join TIEN_TIN on TIEN_TIN.MaTT = LOP.MaTT
	join BIEN_LAI on BIEN_LAI.MSV = SINH_VIEN.MSV
where BIEN_LAI.MaKyHoc = HOC_PHAN.MaKyHoc
and CT_DOI_TUONG.MaKyHoc = HOC_PHAN.MaKyHoc
group by HOC_PHAN.MaKyHoc, SINH_VIEN.MSV
-- 
select HOC_PHAN.MaKyHoc, 
	SINH_VIEN.MSV, 
...
where BIEN_LAI.MaKyHoc = HOC_PHAN.MaKyHoc
and CT_DOI_TUONG.MaKyHoc = HOC_PHAN.MaKyHoc
group by HOC_PHAN.MaKyHoc, SINH_VIEN.MSV, BIEN_LAI.TienNop
order by SINH_VIEN.MSV
--
select SINH_VIEN.MSV, 
	SUM(HOC_PHAN.SoTC * TIEN_TIN.TienTC) as 'hocPhi',
	SUM(HOC_PHAN.SoTC * TIEN_TIN.TienTC * (1 - DOI_TUONG.MucGiam)) as 'TienCanDong',
	SUM(BIEN_LAI.TienNop)
from HOC_TAP
	join SINH_VIEN on SINH_VIEN.MSV = HOC_TAP.MSV
	join HOC_PHAN on HOC_PHAN.MaHP = HOC_TAP.MaHP
	join CT_DOI_TUONG on CT_DOI_TUONG.MSV = SINH_VIEN.MSV
